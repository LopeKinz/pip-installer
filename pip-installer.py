...
def install_package(package, version=None, combined=False):
    ...
    try:
        if version:
            subprocess.run(
                ['pip', 'install', f'{package}=={version}'], check=True)
    ...
    try:
        ...
        while True:
            ...
            if choice == '1':
                ...
                try:
                    subprocess.run(
                        ['pip', 'uninstall', package_name], check=True)
                    num_updated_installed += 1
                except subprocess.CalledProcessError:
                    errors += 1
                ...
                requirements = input(
                    "Enter the package requirements (comma-separated): ").split(',')
                main_file_path = input(
                    "Enter the path to the main.py file (leave blank to use default content): ")

                # Create the package
                create_package(author, name, version,
                               requirements, main_file_path)
